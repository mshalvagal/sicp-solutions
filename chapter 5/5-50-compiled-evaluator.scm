...
;; --- Global environment primitives ---
;; These are looked up by the compiled code via (op lookup-variable-value).
;; Every raw Scheme primitive that the metacircular evaluator source uses
;; directly must be here.
...
(define primitive-procedures
  (list
   ...
   ;; apply (needed for apply-in-underlying-scheme)
   (list 'apply (lambda (proc args) (apply (cadr proc) args)))
   ))

(define (get-compiled-global-environment)
  (let ((initial-env
         (extend-environment
...
;; --- Machine operations ---
;; These are what the compiler emits as (op ...) in the instructions.
 
(define compiled-evaluator-operations
  (list
   ...
   ;; Primitive procedure operations
   ...
   ;; Open-coded primitives (emitted directly by the compiler)
   (list '+ +)
   (list '- -)
   (list '* *)
   (list '= =)
   ...
   ;; Environment access
   (list 'get-global-environment get-compiled-global-environment)
   ))



(define compiled-evaluator-text
  (statements
   (compile-efficient
    '(begin
...
(define compiled-evaluator-machine
  (make-machine
   compiled-evaluator-operations
   (append '((assign env (op get-global-environment)))
           compiled-evaluator-text)))

(start compiled-evaluator-machine)
