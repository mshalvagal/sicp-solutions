...
(define (metacircular-apply procedure arguments env)
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (let* ((param-arg-list (build-param-arg-list (procedure-parameters procedure)
                                                      arguments
                                                      env))
                (params (map car param-arg-list))
                (args (map cadr param-arg-list)))
           (eval-sequence
...
(define (process-param-arg-pair param arg env)
  (cond ((lazy-param? param)
        ...
        (else (list param
                    (metacircular-eval arg env)))))

(define (build-param-arg-list params args env)
  (if (null? params)
      '()
      (let ((first-param (car params))
            (first-arg (car args)))
        (cons (process-param-arg-pair first-param first-arg env)
              (build-param-arg-list (cdr params) (cdr args) env)))))
...
(display "Testing (call (lambda (a (b lazy)) (if (call = a 0) -42 b)) 0 (call / 1 0))")(newline)
(metacircular-eval '(call (lambda (a (b lazy))
                            (if (call = a 0) -42 b))
                          0
                          (call / 1 0))
...
(newline)(newline)
(display "Exercise 4.31")(newline)
(display "I solved this by just changing the metacircular-apply function. Specifically, I processed the param and arg list to delay what needs to be delayed before extending the environment. Was fairly simple in the end.")
