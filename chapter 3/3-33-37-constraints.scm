...
(define (averager x1 x2 avg)
  (let ((denom (make-connector))
        (sum (make-connector)))
    (constant 2 denom)
    (multiplier avg denom sum)
    (adder x1 x2 sum)
    'ok))

; sqrt and other stuff for 3.34 and 3.35
(define (square x) (* x x))
...
(define x2_square (make-connector))

(averager x1 x2 avg)
(constant 1 x1)
(squarer x2 x2_square)

(probe 'input2 x2)
(probe 'input2squared x2_square)
(probe 'fahrenheit F)

(set-value! avg 4 'user)
(forget-value! avg 'user)

(set-value! avg 8 'user)
